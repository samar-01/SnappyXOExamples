// This code controls a car with a steering mechanism via bluetooth. You can control the speed and steer angle of the car.
#include <Servo.h>
#include <MicroBlue.h>
...
void setup() {
  ...
  // Library forward sets InputOne(4)=LOW, InputTwo(7)=HIGH — opposite on the shared
  // pin 4, so invert the left motor so logical "forward" matches original behaviour.
...
void loop() {
  ...
  if (msg.id == "d0" || msg.id == "d1") {
    ...
    if (throttle > 0)       forward(mappedThrottle);
    else if (throttle == 0) pause();
    else                    reverse(mappedThrottle);
