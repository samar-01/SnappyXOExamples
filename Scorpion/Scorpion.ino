// This project aims to educate the user on how to control a 2WD car using an Ultrasonic Sensor as well as an catapult mechanism using finite servos.
// Include libraries
#include <Servo.h>
...
void loop() {
  ...
  /* check for distance from object and random integer to do the following:
     reverse -> pause -> trigger catapult -> turn left/right based on randint -> go forward -> restart loop
  */
  if ((cm < 18) && (randint == 0)) {
    REVERSE();
    delay(500);
