// Piano Project Code This project aims to teach the user about the ultrasonic sensor avialable in the Advanced Robotics SnappyXO kit. When a key is pressed on the piano, a paddle blocks the ultrasonic sensor. Then, according to the distance that paddle is away from the sensor, a tone is played with a buzzer.
// Pin Definitions 
#define trigger 9
#define echo 10
...
const float OMEGA_C = 50.0/(2.0*3.1415);  // [Hz], Cut-off frequency for low pass filter
...
int NOTE_A4 = 440;
int NOTE_C5 = 523;
float C_nom = 7; // [cm], Nominal values for the distance of each paddle from emperical trials
float D_nom = 13;
float E_nom = 20;
float backStop = 25;
...
void loop() {
  ...
  if ((currentMillis - prevMillis) > T) {
    ...
    // Calculate distance using speed of sound and duration and filter the data
    ...
    if ( CID < allowableCID && data < backStop){
      ...
      if (fdata <= C_nom + allowableRange && fdata >= C_nom - allowableRange){
        NOTE = NOTE_C4;
      }
      else if (fdata <= D_nom + allowableRange && fdata >= D_nom - allowableRange){
        NOTE = NOTE_D4;
      }
      else if (fdata <= E_nom + allowableRange && fdata >= E_nom - allowableRange){
        NOTE = NOTE_E4;
      }
      else {
        NOTE=0;
...
float filteredData(float DATA,float OMEGA_C,float T,float FDATA_KM1,float DATA_KM1) {
  ...
  float FDATA = (1-OMEGA_C*T_sec)*FDATA_KM1 + OMEGA_C*T_sec*DATA_KM1;
  return FDATA;
}