// This code allows the 2WD DC motor car to avoid obstacles using an Ultrsonic sensor and lights up an LED that changes color based on the direction that the car is moving
...
void setup() {
  ...
  // Initialise motors (no inversion needed — hardware rewired to match library pins)
...
void loop() {
  ...
  forward();
  green();
  randint = random(0,2);// // randomly choose whether to turn left or right if an obstacle is encountered; in this code, 0 is turning right and 1 is turning left
