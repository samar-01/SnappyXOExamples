// This code aims to educate the user on how to use various sensors through an interactive game. User inputs affect the readings of each sensor and push the user to test their reaction time.
// Pin Definitions
int buzzerPin = 6;
int photoLEDPin = 7;
int switchLEDPin = 8;
...
void setup(){
 ...
 randomSeed(analogRead(A5)); // Reads an open pin
  ...
  while (digitalRead(buttonPin) == HIGH || digitalRead(potPin) == HIGH || digitalRead(switchPin) == HIGH) {
